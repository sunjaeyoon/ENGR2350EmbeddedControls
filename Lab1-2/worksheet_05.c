...
void Port_Init(void)
{
    ...
    // Port 2
    P2MDOUT &= ~0x01; // set input pin P2.0 in open drain mode 
...
void Interrupt_Init(void)
{
    IE |= 0x82; //enable Timer0 interrupts by setting the appropriate bit in the SFR
...
void Timer_Init(void) 
{
    ...
    TMOD |= 0x00;   // Leave T1 intact and set T0 mode 13bit
    TR0 = 0;    // Stop Timer0 
...
// or use 2 commands for low and high bytes separately 
    //TL0 ____________;    // Clear low byte of register T0 
    //TH0 ____________;    // Clear high byte of register T0 
}
